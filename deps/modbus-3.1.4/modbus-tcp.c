/*
 * Copyright 漏 2001-2013 St茅phane Raimbault <stephane.raimbault@gmail.com>
 *
...
# endif
/*当版本小于xp时，getaddrino freeaddrinfo会未定义*/
...
#include "../modbus-3.1.4/modbus-tcp.h"
#include "../modbus-3.1.4/modbus-tcp-private.h"
...
modbus_t* modbus_new_tcp(const char *ip, int port)
{
	...
	//comNumber
	strcpy(ctx->_comNumber, "");

    return ctx;
}

modbus_t* modbus_new_tcp_lm(const char *comNumber, const char *ip, int port)
{
	modbus_t *ctx;
	modbus_tcp_t *ctx_tcp;
	size_t dest_size;
	...
	if (comNumber == NULL || *comNumber == 0) {
		fprintf(stderr, "The comNumber string is empty\n");
		errno = EINVAL;
		return NULL;
	}
	...
	ctx_tcp = (modbus_tcp_t *)ctx->backend_data;
	//comNumber
	memset(ctx->_comNumber, 0, sizeof(char) * 30);
	strcpy(ctx->_comNumber, comNumber);
...
modbus_t* modbus_new_rtutcp_lm(const char *comNumber, const char *ip, int port)
{
	modbus_t *ctx;
	modbus_tcp_t *ctx_tcp;
	size_t dest_size;
