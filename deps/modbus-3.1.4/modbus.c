...
#include "../modbus-3.1.4/config.h"

#include "../modbus-3.1.4/modbus.h"
#include "../modbus-3.1.4/modbus-private.h"
...
//         }
		if (context != NULL) {
			LOGDEBUG("ERROR %s: %s", modbus_strerror(errno),context);
		}
		else {
			LOGDEBUG("ERROR %s", modbus_strerror(errno));
		}
    }
}
...
static int send_msg(modbus_t *ctx, uint8_t *msg, int msg_length)
{
    ...
    if (ctx->debug) {
		...
		char bufTime[32];
		bufTime[strftime(bufTime, sizeof(bufTime), "%Y-%m-%d %H:%M:%S", lt)] = '\0';
		printf("%s ", bufTime);
		log_comm2(ctx->_comNumber, "Tx", "");
    ...
    do {
        ...
        if (rc == -1) {
            ...
            if (ctx->error_recovery & MODBUS_ERROR_RECOVERY_LINK) {
				...
				LOGDEBUG("send_msg:errno=%d", saved_errno);
...
#ifdef WIN32
					if (strncasecmp(ctx->_comNumber,"com",3)!=0)	//不是rtu
					{
						modbus_close(ctx);
						_sleep_response_timeout(ctx);
						modbus_connect(ctx);
...
//    }
    ...
    while (length_to_read != 0) {
        ...
        if (rc == -1) {
            ...
            if (ctx->error_recovery & MODBUS_ERROR_RECOVERY_LINK) {
                int saved_errno = errno;
				LOGDEBUG("receive_msg select:errno=%d", saved_errno);
#ifdef WIN32
				if (strncasecmp(ctx->_comNumber, "com", 3) != 0)	//不是rtu
				{
					modbus_close(ctx);
...
#else
                if (errno == ETIMEDOUT) {
					LOGERR("receive message timeout");
                    _sleep_response_timeout(ctx);
                    modbus_flush(ctx);
					//lm_add
// 					modbus_close(ctx);
        ...
        if (rc == -1) {
            ...
            if ((ctx->error_recovery & MODBUS_ERROR_RECOVERY_LINK) &&
                (errno == ECONNRESET || errno == ECONNREFUSED ||
                 errno == EBADF)) {
                int saved_errno = errno;
				LOGDEBUG("receive_msg recv:errno=%d", saved_errno);
                modbus_close(ctx);
                modbus_connect(ctx);
...
//                         function, req[offset]);
				LOGERR("Received function not corresponding to the request (0x%X != 0x%X)",function, req[offset]);
            }
            if (ctx->error_recovery & MODBUS_ERROR_RECOVERY_PROTOCOL) {
                _sleep_response_timeout(ctx);
                modbus_flush(ctx);
...
//                         rsp_nb_value, req_nb_value);
				LOGERR("Quantity not corresponding to the request (%d != %d)",rsp_nb_value, req_nb_value);
            }
...
//                     rsp_length, rsp_length_computed);
			LOGERR("Message length not corresponding to the computed length (%d != %d)",rsp_length, rsp_length_computed);
        }
        if (ctx->error_recovery & MODBUS_ERROR_RECOVERY_PROTOCOL) {
            _sleep_response_timeout(ctx);
            modbus_flush(ctx);
...
/* Send a response to the received request.
   ...
   accordingly.
*/
int modbus_reply_lm(modbus_t *ctx, const uint8_t *req,
                 int req_length, modbus_mapping_t *mb_mapping,int* _write_mapping_address, int *_write_mapping_number)
{
    int offset;
    int slave;
    int function;
    ...
    case MODBUS_FC_WRITE_SINGLE_COIL: {
        ...
        if (mapping_address < 0 || mapping_address >= mb_mapping->nb_bits) {
			...
			if(_write_mapping_address!=NULL)	//lm_add	
				*_write_mapping_address = mapping_address+1;
			if (_write_mapping_number != NULL)
				*_write_mapping_number = 1;
            } else {
                rsp_length = response_exception(
                    ctx, &sft,
    ...
    case MODBUS_FC_WRITE_SINGLE_REGISTER: {
        ...
        if (mapping_address < 0 || mapping_address >= mb_mapping->nb_registers) {
            ...
            rsp_length = req_length;
			if(_write_mapping_address!=NULL)	//lm_add	
				*_write_mapping_address = mapping_address+400001;
			if (_write_mapping_number != NULL)
				*_write_mapping_number = 1;
        }
    }
...
//                     nb, MODBUS_MAX_READ_BITS);
			LOGERR("ERROR Too many bits requested (%d > %d)",nb, MODBUS_MAX_READ_BITS);
        }
        errno = EMBMDATA;
        return -1;
    }
...
//                     nb, MODBUS_MAX_READ_BITS);
			LOGERR("ERROR Too many discrete inputs requested (%d > %d)",nb, MODBUS_MAX_READ_BITS);
        }
        errno = EMBMDATA;
        return -1;
    }
...
//                     nb, MODBUS_MAX_READ_REGISTERS);
			LOGERR("ERROR Too many registers requested (%d > %d)",nb, MODBUS_MAX_READ_REGISTERS);
        }
        errno = EMBMDATA;
        return -1;
    }
