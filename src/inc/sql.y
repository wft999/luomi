...
////////////////////////////////////////gate mangant/////////////////////////////////////////
cmd ::= CREATE GATE ids(X) insert_value_list(K). {
  ...
  pInfo->sqlType = CREATE_GATE;
}

//cmd ::= CREATE GATE ids(X) VALUES LP  RP.        {setDCLSQLElems(pInfo, CREATE_GATE, 1, &X);}		
//cmd ::= ALTER TABLE ids(X) SET ids(Y) EQ tagitem(Z).			
