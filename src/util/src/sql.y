...
////////////////////////////////////////gate mangant/////////////////////////////////////////
cmd ::= CREATE GATE ids(X) STRING(Y). {setDCLSQLElems(pInfo, CREATE_GATE, 2, &X, &Y);}
cmd ::= QUERY GATE ids(X).            {setDCLSQLElems(pInfo, QUERY_GATE, 1, &X);}
cmd ::= UPDATE GATE ids(X) STRING(Y). {setDCLSQLElems(pInfo, UPDATE_GATE, 2, &X, &Y);}
cmd ::= DELETE GATE ids(X).           {setDCLSQLElems(pInfo, DELETE_GATE, 1, &X);}
